        ...
        NodeFactory
{
    ...
    private void parseJsTxt(int start) throws ParserException
    {
        mNextNode = parseString(start, true);
    }
        ...
        switch (ch)
        {
            ...
            case '<':
                ...
                else if ('/' == ch || '%' == ch || Character.isLetter (ch))
                {
                    mPage.ungetCharacter (mCursor);
                    ret = parseTag (start);
                    if (((TagNode)ret).getTagName().equalsIgnoreCase("script"))
                    {
                        parseJsTxt(mCursor.getPosition());
                    }
                }
                else if ('!' == ch)
                {
